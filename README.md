...
ローカルのタスクスケジューラ（`setup_scheduled_task.ps1`）は使わない。実行すると
GitHub Actions と二重に動く。

### アカウントと名義

グループ全アカウントの予約が対象。**取消は持ち主のアカウントでしかできない**ので
そのつど切り替え、**取り直しは常にマスター**で行う。窓口へ行くのは本人だけなので、
乗り換えや取り直しのたびに名義が本人へ寄っていく。

アカウント一覧はDB（`*.sqlite`）にあるが、これはリポジトリに含めていない。
DBを引けない環境（GitHub Actions）では `OGURI_MEMBER_IDS`（カンマ区切りの
アカウントID）から組み立てる。パスワードは全員 `NETAICHI_DEFAULT_PW` 共通なので
IDだけで足りる。未設定ならマスターだけが対象になる。
...
- **フットサルのように予約一覧に面番号が出ないコートが混じる枠も対象外**。
